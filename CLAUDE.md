...
- **A host page's `Permissions-Policy` header can veto the panel's camera and mic outright, and `allow=` cannot override it.** Measured 2026-09-01: claude.ai sends `camera=(),microphone=()` — an *empty* allowlist. Delegation only ever **narrows** what the parent document already holds, so a parent holding nothing has nothing to hand down and the iframe's `allow="camera; microphone"` becomes a no-op. Symptom is a console `[Violation] Permissions policy violation: camera is not allowed in this document.` followed by `handshake failed: NotAllowedError: Permission denied` from `openDevices`. **This is a different gate from the Session 3E prompt problem and `permission.html` cannot fix it** — policy is checked first, so Chrome never reaches the stored grant. Distinguishing them: the 3E failure produces *no* violation line, this one always does. Diagnose with `curl -sS -D - -o /dev/null <url> | grep -i permissions-policy` — a site that doesn't list camera/microphone at all (YouTube) leaves them at their default allowlist of `self`, which delegates fine. **Check this before debugging anything else when the panel can't open devices on a new site.** Fixable in principle by stripping the header with `declarativeNetRequest` `modifyHeaders`; not built, and not needed for sites anyone would actually watch on.
- **Sites can fight injected UI.** z-index wars, CSP, layout assumptions. YouTube behaves. Test the others before committing.
- **`Extension context invalidated` means a stale content script, not a bug.** The extension was reloaded while an old content script was still live in an already-open tab. Close and reopen the tab — a plain refresh does not always clear it. Expect this constantly given the two-step reload loop.
